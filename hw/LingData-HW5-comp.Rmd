...
```{r 1.1}
ggplot(shva, aes(f2, f1, color = vowel)) + geom_point() + scale_x_continuous(trans = 'reverse') + scale_y_continuous(trans = 'reverse')
```
...
```{r 1.2}
ggplot(shva, aes(vowel, f1, fill=vowel)) + geom_boxplot(outlier.color = NULL) + coord_flip()
ggplot(shva, aes(vowel, f2, fill=vowel)) + geom_boxplot(outlier.color = NULL) + coord_flip()
```

### 1.3 
Calculate Pearson correlation of `f1` and `f2` (all data)
```{r 1.3}
cor.test(shva$f1, shva$f2, method = 'pearson')
```
                                                                 
### 
1.4 Calculate Pearson correlation of `f1` and `f2` for each vowel
```{r 1.4}
A <- filter(shva, vowel == 'A')
cor.test(A$f1, A$f2, method = 'pearson')
y <- filter(shva, vowel == 'y')
...
```{r 2.1.4}
ggplot(shva, aes(f2, f1, color = vowel)) + geom_point() + scale_x_continuous(trans = 'reverse') + 
  scale_y_continuous(trans = 'reverse') + geom_smooth(method = "lm")
```
...
```{r 2.1.5}
ggplot(shva, aes(f2, f1, color = vowel)) + geom_point() + scale_x_continuous(trans = 'reverse') +
  scale_y_continuous(trans = 'reverse') + facet_wrap(~vowel)
```

### 2.2.1 
Use the linear regression model to predict `f2` by `f1` and `vowel`.
```{r 2.2.1}
predf2 <- lm(data = shva, f2 ~ f1 + vowel)
summary(predf2)
```
...
```{r 2.2.2}
summary(predf2)$coefficients[1]
```

### 2.2.3 
Provide the adjusted R$^2$
```{r 2.2.3}
summary(predf2)$adj.r.squared
```
...
```
Vowels and f1 are not enough to predict f2 with high precision; however, using them for predicting f2 simultaneously gives better results than when using just one of them.
```
...
Data from Natalya Levshinas `RLing` package available (here)[https://raw.githubusercontent.com/agricolamz/2018-MAG_R_course/master/data/dutch_causatives.csv]
...
If we try chisq.test() with Aux and CeSynt, we`ll get an error. The p-value < 2.2e-16, making the association statistically significant.
...
p-value is small enough to discard the initial hypothesis.
...
```{r 3.4}
ct <- chisq.test(d_caus$Aux, d_caus$EPTrans)$observed
or <- (ct[1, 1] / ct[2, 1]) / (ct[1, 2] / ct[2, 2])
or
```
...
```{r 3.4.1}
ct2 <- chisq.test(d_caus$Aux, d_caus$EPTrans)$expected
or2 <- (ct2[1, 1] / ct2[2, 1]) / (ct2[1, 2] / ct2[2, 2])
or2
```
What can you say about odds ratio of expected frequencies for arbitrary data?
  
It equals 1 because we assume that the events observed are independent.
                                                                   
### 3.5 
Calculate effect size for this test using Cramer`s V (phi).
```{r 3.5}
library(lsr)
cramersV(chisq.test(d_caus$Aux, d_caus$EPTrans)$observed)
```

### 3.6. 
Report the results of independence test using the following template:
```
We have not found a significant association between variables Aux and EPTrans (p > 0.001).  The odds of intransitive predicate were 2.606162 times higher in doen than in latem. Effect size is small (Cramer`s V = 0.1691587).
```
...
```
Because the chance to make a statistical error grows with the number of added values.
```

### 3.9 
Provide a short text (300 words) describing the hypothesis of this study and the results of your analysis.
```{r 3.9}
The zero hypothesis of the study was the assumption that usage of doen and laten is not connected to the semantic 
classes of causers and causees, the semantic domains of the caused event, the syntactic statuses of the causee,
transitivity/intransitivity, region or the topic. The alternative hypothesis is the significance of any of these
factors. By using logistic regression, checking the odds and conducting various tests, it became possible to 
measure the real impact of these factors. I found out that there is no significant association between variables Aux and EPTran.
Also, it was calculated that the odds of intransitive predicate were 2.606162 times higher in doen than in laten.
```
